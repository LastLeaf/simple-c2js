...
var convertType = (targetType, sourceType, body) => {
  ...
  // TODO |0 is not needed for normalized int
  ...
  if(targetTypeName === 'int' && sourceTypeName === 'double') return { op: OP.BIT_REVERT, body: { op: OP.BIT_REVERT, body: { op: OP.CALL, name: 'floor', body: [body] } } }
...
var multiWithType = (item, info, funcInfo) => {
  ...
  if(typeInfo.name === 'int') {
    // for integers
    if(item.op === '*') {
      let treeLeft = convertType(typeInfo, left.type, left.tree)
      let treeRight = convertType(typeInfo, right.type, right.tree)
      return {
        type: typeInfo,
