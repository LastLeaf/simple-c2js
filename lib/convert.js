...
var convertType = (targetType, sourceType, body) => {
  ...
  if(targetTypeName === 'float' && sourceTypeName === 'int') return froundTree({ op: OP.BIT_OR, left: body, right: LITERAL_ZERO })
  if(targetTypeName === 'float' && sourceTypeName === 'double') return froundTree(body)
  if(targetTypeName === 'double' && sourceTypeName === 'int') return { op: OP.POSITIVE, body: { op: OP.BIT_OR, left: body, right: LITERAL_ZERO } }
...
var memberOpWithType = function(item, info, funcInfo){
  ...
  item.body.forEach((item, i) => {
    ...
    if(item.op === '[]') {
      ...
      offsetArr.push(imulTree(size, childTree))
    } else {
...
var defStatement = (item, info, funcInfo) => {
  ...
  item.body.forEach((item) => {
    ...
    if(valueInfo) {
      ...
      funcInfo.vars[name] = {
        rename: mangleNameByIndex(funcInfo.nameCount++),
        type: typeInfo,
      ...
      initArr.push({ op: OP.ASSIGN, left: { op: OP.VAR, body: name }, right: convertType(typeInfo, exp.type, exp.tree) })
...
var ifWithReturnInfo = (item, info, funcInfo) => {
  var cond = expWithType(item.cond, info, funcInfo).str
  ...
  return {
    hasReturn: left.hasReturn && (!right || right.hasReturn),
    tree: {
      op: OP.IF,
      cond,
      left,
...
var whileWithReturnInfo = (item, info, funcInfo) => {
  var cond = expWithType(item.cond, info, funcInfo).str
  var {hasReturn, tree} = sentenceOrBlockWithReturnInfo(item.body, info, funcInfo)
  return {
    hasReturn,
    tree: {
      op: OP.WHILE,
...
var forWithReturnInfo = (item, info, funcInfo) => {
  var init = item.init ? expWithType(item.init, info, funcInfo).tree : ''
  var cond = item.cond ? expWithType(item.cond, info, funcInfo).tree : ''
  var step = item.step ? expWithType(item.step, info, funcInfo).tree : ''
  var {hasReturn, tree} = sentenceOrBlockWithReturnInfo(item.body, info, funcInfo)
  return {
    hasReturn,
    tree: {
...
var parseStatic = (item, info) => {
  ...
  item.body.forEach((item) => {
    ...
    var rename = mangleNameByIndex(info.nameCount++)
    info.statics[name] = {
      rename,
      type: typeInfo,
      value: valueInfo.value,
...
var parseFunction = (item, info) => {
  ...
  var funcInfo = info.functions[name] = {
    rename: mangleNameByIndex(info.nameCount++),
    type: typeInfo,
    args,
    argsOrder,
    nameCount,
