...
var LITERAL_ONE = { op: OP.LITERAL, body: '1' }

// generate built-in functions and consts
var builtInFunctions = {}
var builtInConsts = {}
var initBuildIn = function(){
  for(var cName in STDLIB_TYPES) {
    var rename = '__' + cName
    var typeName = STDLIB_TYPES[cName][1]
    var typeInfo = typeName === 'int' ? INT_TYPE : DOUBLE_TYPE
    var arg1TypeName = STDLIB_TYPES[cName][2]
    var arg2TypeName = STDLIB_TYPES[cName][3]
    if(!arg1TypeName) {
      builtInConsts[cName] = {
        rename,
        type: typeInfo,
      ...
      args.a = {
        rename: 'a',
        type: arg1TypeName === 'int' ? INT_TYPE : DOUBLE_TYPE,
        body: null
      }
      argsOrder.push('a')
      if(arg2TypeName) {
        args.b = {
          rename: 'b',
          type: arg2TypeName === 'int' ? INT_TYPE : DOUBLE_TYPE,
          body: null
        }
        argsOrder.push('b')
      }
      builtInFunctions[cName] = {
        rename,
        type: typeInfo,
        args,
        argsOrder,
...
var getVarStrWithType = (item, name, info, funcInfo) => {
  var varInfo = funcInfo.vars[name] || funcInfo.args[name] || info.statics[name] || builtInConsts[item.name]
...
var convertTypeAndDef = (targetType, sourceType, value) => {
  ...
  if(targetTypeName === 'int') return { op: OP.LITERAL, body: String(value|0) }
...
var convertType = (targetType, sourceType, body) => {
  ...
  if(targetTypeName === 'int') {
    ...
    return { op: OP.BIT_REVERT, body: { op: OP.BIT_REVERT, body } }
  }
  // FIXME convert unsigned int to float/double
  if(targetTypeName === 'double') {
  ...
  // FIXME type def may not has location
...
var commaWithType = (item, info, funcInfo) => {
  throw new CompilationError('Comma expressions are currently not implemented')
}
...
var conditionWithType = (item, info, funcInfo) => {
  throw new CompilationError('Condition expressions are currently not implemented')
}
...
var bitWithType = (item, info, funcInfo) => {
  ...
  var right = expWithType(item.right, info, funcInfo)
  var op = OP.BIT_XOR
  if(item.op !== '^') op = item.op === '&' ? OP.BIT_AND : OP.BIT_OR
  return {
    type: INT_TYPE,
    tree: {
      op,
      left: convertType(INT_TYPE_UNNORMALIZED, left.type, left.tree),
      right: convertType(INT_TYPE_UNNORMALIZED, right.type, right.tree),
    }
  }
}

var logicWithType = (item, info, funcInfo) => {
  var bit = bitWithType(item, info, funcInfo)
  return {
    type: INT_TYPE,
    tree: {
      op: OP.BIT_AND,
...
var shiftWithType = (item, info, funcInfo) => {
  var left = expWithType(item.left, info, funcInfo)
  var right = expWithType(item.right, info, funcInfo)
  var op = OP.LEFT_SHIFT
  if(item.op === '>>') {
    op = left.type.unsigned ? OP.UNSIGNED_RIGHT_SHIFT : OP.RIGHT_SHIFT
  }
  return {
    type: left.type.unsigned ? UNSIGNED_INT_TYPE : INT_TYPE,
    tree: {
      op,
      left: convertType(INT_TYPE_UNNORMALIZED, left.type, left.tree),
      right: convertType(INT_TYPE_UNNORMALIZED, right.type, right.tree),
...
var addWithType = (item, info, funcInfo) => {
  ...
  if(left.type.pointer) {
    ...
    unrefType.pointer --
    let retTypeInfo = cloneTypeInfo(left.type)
    retTypeInfo.unnormalized = true
    return {
      type: retTypeInfo,
      tree: {
  ...
  var typeInfo = alignType(left.type, right.type)
  var retTypeInfo = cloneTypeInfo(typeInfo)
  if(typeInfo.name !== 'double') retTypeInfo.unnormalized = true
  return {
    type: retTypeInfo,
    tree: {
      op: item.op === '+' ? OP.PLUS : OP.MINUS,
...
var unaryWithType = (item, info, funcInfo) => {
  ...
  if(item.op === '+n') {
    throw new CompilationError('Unary add is currently not supported', item.body.location)
  }
  if(item.op === '-n') {
    if(typeInfo.name === 'int') {
      typeInfo = cloneTypeInfo(typeInfo)
    ...
    return {
      ...
      tree: {
        op: OP.NEGATIVE,
        body: typeInfo.name === 'int' ? convertType(INT_TYPE, child.type, child.tree) : child.tree,
      }
    }
  }
  if(item.op === '~n') {
    return {
      ...
      tree: {
        op: OP.BIT_REVERT,
        body: convertType(INT_TYPE_UNNORMALIZED, typeInfo, child.tree)
      }
    }
  }
  if(item.op === '!n') {
    return {
      ...
      tree: {
        op: OP.REVERT,
        body: convertType(INT_TYPE, typeInfo, child.tree)
      }
    }
  }
}
...
var callWithType = (item, info, funcInfo) => {
  ...
  var func = info.functions[item.name] || builtInFunctions[item.name]
...
var expWithType = (item, info, funcInfo) => {
  ...
  if(item.op === '==' || item.op === '!=' || item.op === '<' || item.op === '>' || item.op === '<=' || item.op === '>=') return relationWithType(item, info, funcInfo)
...
var returnStatement = (item, info, funcInfo) => {
  ...
  var body = convertType(funcType, type, tree)
  if(funcType.name === 'int' && type.name === 'int') body = { op: OP.BIT_REVERT, body: { op: OP.BIT_REVERT, body } }
  return { op: OP.RETURN, body }
}
