...
var imulStr = (str1, str2) => {
  return STDLIB_PRESERVE['Math.imul'] + '((' + str1 + '),(' + str2 + '))'
}
...
var convertTypeStr = function(targetType, sourceType, str){
  ...
  if(targetTypeName === sourceTypeName) {
    if(sourceType.unnormalized && !targetTypeName.unnormalized) {
      if(targetTypeName === 'int') return '(' + str + ')|0'
      if(targetTypeName === 'float') return froundStr(str)
      if(targetTypeName === 'double') return '+(' + str + ')'
    }
    return str
  }
...
var generateMemberOpWithType = function(item, info, funcInfo){
  ...
  item.body.forEach((item, i) => {
    if(i === 0) return
    if(item.op === '[]') {
      let child = generateExpWithType(item.body, info, funcInfo)
      if(!typeInfo.pointer) {
      ...
      offsetArr.push(imulStr(size, childStr))
    } else {
  ...
  return {
    type: typeInfo,
    str: heapType + '[(' + name + '|0' + (offsetArr.length ? '+' + offsetArr.join('+') : '') + (heapShift ? ')>>' + heapShift + '<<' + heapShift + '>>' + heapShift : ')') + ']'
  }
}

// expressions

var generateAssignmentWithType = (item, info, funcInfo) => {
  var left = item.left.op === '=' ? generateVarWithType(item.left, info, funcInfo) : generateMemberOpWithType(item.left, info, funcInfo)
  var right = generateExpWithType(item.right, info, funcInfo)
  return {
    type: left.type,
...
var generateAddWithType = (item, info, funcInfo) => {
  // TODO
}

var generateMultiWithType = (item, info, funcInfo) => {
  // TODO
}

var generateExpWithType = (item, info, funcInfo) => {
  // TODO
  if(item.op === '=') return generateAssignmentWithType(item, info, funcInfo)
  if(item.op === '+-') return generateAddWithType(item, info, funcInfo)
  if(item.op === '*/%') return generateMultiWithType(item, info, funcInfo)
...
var generateReturn = (item, info, funcInfo) => {
  ...
  var {type, str} = generateExpWithType(item.body, info, funcInfo)
...
var generateDef = (item, info, funcInfo) => {
  ...
  item.body.forEach((item) => {
    ...
    if(valueInfo) {
      ...
      let exp = generateExpWithType(item.value, info, funcInfo)
...
var generateForWithReturnInfo = (item, info, funcInfo) => {
  var initStr = item.init ? generateExpWithType(item.init, info, funcInfo).str : ''
  var condStr = item.cond ? generateExpWithType(item.cond, info, funcInfo).str : ''
  var stepStr = item.step ? generateExpWithType(item.step, info, funcInfo).str : ''
  var {hasReturn, bodyStr} = generateSentenceOrBlockWithReturnInfo(item.body, info, funcInfo)
  return {
    hasReturn,
...
var generateSentenceWithReturnInfo = (item, info, funcInfo) => {
  switch(item.op) {
    ...
    default:
      return {
        hasReturn: false,
        bodyStr: generateExpWithType(item, info, funcInfo).str + ';'
      }
  }
}
