...
var stringifyFunctions = (tree) => {
  ...
  for(let k in functions) {
    ...
    var argDef = ''
    for(let k in tree.args) {
      argDef = tree.args[k].rename + '=' + stringifyExp(tree.args[k].body) + ';'
    }
    var varDef = ''
    for(let k in tree.vars) {
      varDef = 'var ' + tree.args[k].rename + '=' + stringifyExp(tree.args[k].body) + ';'
    }
    retArr.push('function ' + func.rename + '(' + func.argsOrder.join(',') + '){' + argDef + varDef + stringifyBlock(func.body) + '}')
  }
  return retArr.join('')
}
