...
typeName
  = name:('int' / 'double' / 'float' / 'char' / 'void') pointer:[*]* { return { location: location(), struct: false, name: name, pointer: pointer.length } }
  / 'struct' ws name:varName pointer:[*]* { return { location: location(), struct: true, name: name, pointer: pointer.length } }
