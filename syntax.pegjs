...
assignmentExp
  = left:lvalue ws? '=' right:expression { return { op: '=', left: left, right: right } }
  / conditionExp
...
additionNext
  = ws? op:[-+] item:multiplication next:additionNext { return next.unshift({ op: op, body: item }), next }
  / '' { return [] }
...
multiplicationNext
  = ws? op:[*/%] item:unaryExp next:multiplicationNext { return next.unshift({ op: op, body: item }), next }
  / '' { return [] }
